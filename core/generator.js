...
5. 浏览历史是检索的影子。一部分检索会带 1〜2 条「点进去的页面」(visits):标题像真实网页(Q&A、まとめ、攻略 wiki、商品评测、匿名掲示板のスレ,以及日本网络特产的诊断/占い/恋愛コラム/「〜な人の特徴10選」式ランキング记事那一挂——心里有事的人深夜最容易点进相性診断),站名是这个世界里的网站、贴合世界观,不写现实世界的真实网站名;另一部分浏览是与心事无关的日常惯性(天气、新闻、兴趣、购物),让历史像真人的手机。
6. {{LANG_RULE}}
...
3. 极致的日式网页复刻。调动你对日本互联网的全部语料记忆,精准还原各流派的版式气质与特有文案:匿名掲示板(>>安価、名無し、极简灰线)/まとめ(红蓝特大粗体、大面积引用块、假广告位、「いかがでしたか?」)/Q&A(ベストアンサー标识、提问与回答的气泡)/クチコミ评测(CSS 星级★★★★☆、「20代女性/敏感肌」式用户属性标签、条形评分)/诊断・占い(夸张的结果大字框与煽り文句)/大百科(曖昧さ回避警告框、带主观吐槽的概要、右侧信息卡)——按站名与标题判断这是哪一挂,大胆用 blockquote/table/details 与内联 <style> 搭 UI 小件(阴影卡片、标签、面包屑、气泡),网页的「土味」与「个性」本身就是趣味。
4. 像完整的网页,不只是一篇文章:带上伪造的站头导航、侧边栏或相关链接区(链接一律 href="#",它们不会被点通)。
5. 技术边界:输出单一 HTML 片段(不含 <html>/<head>/<body> 外壳,从最外层容器 div 直接开始);样式集中写在顶部一个 <style> 块;🚫绝不写 <script>,🚫绝不引用任何外部资源(字体、图库都不行);图片一律用色块 <div> 代替,里面写上素材说明文字(「[画像:頭を抱えるスーツの男性のフリー素材]」式——フリー素材感本身就是日式网页的一大笑点)。
6. 篇幅:一屏到两屏的信息量(正文 300〜800 字级),重心放在首屏视觉冲击与核心内容区,不写巨型长文。
7. {{LANG_RULE}}
