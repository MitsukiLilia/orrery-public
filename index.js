...
export const ORRERY_VERSION = '0.22.1';
...
function main() {
    ...
    async function hasPendingFloors() {
        ...
        const [wmMessenger, wmForum, wmSns, wmBrowser, wmGallery, wmMemo, wmAlmanac] = await Promise.all([
        ...
        return wmMessenger < tip || wmForum < tip || wmSns < tip || wmBrowser < tip || wmGallery < tip || wmMemo < tip || wmAlmanac < tip;
    }
    ...
    async function hasNewRipples() {
        ...
        return hasUnseenInApp('messenger', world, seen) || hasUnseenInApp('forum', world, seen)
            ...
            || hasUnseenInApp('almanac', world, seen);
    }
