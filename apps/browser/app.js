 ...
 *  M11:导出给 apps/almanac/app.js 复用(板块页的列表用同一套日分隔逻辑,不再复制一份)。 */
export function withDateSeps(items, refNow) {
    const out = [];
...
export function renderBrowserHtml({ world, busy, tab = 'search', seenAt = 0, browserNow }) {
    ...
    // M11:内网 lane 收编后 world.visits 里不再有 pinned 常驻卡(fold 已跳过),两 tab 都不必再过滤。
 ...
 */
export function renderWebPageHtml({ visit, snapshot, busy, starred = {}, exportBusy = false }) {
    ...
    if (snapshot) {
        ...
        const zhLines = [snapshot.zh].filter(Boolean);
